...
import com.evotek.iam.application.config.AuthProperties;
...
public class AuthController {
    ...
    public void init() {
        ...
        this.authServiceCommand = authServiceFactory.createAuthServiceCommand(typeAuthCommandService);
        this.authServiceQuery = authServiceFactory.createAuthServiceQuery(typeAuthQueryService);
    }
    ...
    @Operation(
            ...
            responses = {@ApiResponse(responseCode = "200", description = "Gửi mã Otp thành công")})
    ...
    public ApiResponses<TokenDTO> loginIam(@RequestBody LoginRequest loginRequest) {
        ...
        return ApiResponses.<TokenDTO>builder()
                .data(tokenDTO)
                .success(true)
                .code(tokenDTO == null ? 202 : 200)
                .message(tokenDTO == null ? "Cần xác thực mã OTP" : "Đăng nhập thành công")
                .timestamp(System.currentTimeMillis())
                .status(tokenDTO == null ? "NEED_OTP" : "OK")
                .build();
    }
    ...
    @Operation(
            ...
            description = "API này sẽ gửi link reset mật khẩu đến email.",
            requestBody =
                    @io.swagger.v3.oas.annotations.parameters.RequestBody(
            ...
            responses = {@ApiResponse(responseCode = "200", description = "Reset password link sent to email")})
    ...
    public ApiResponses<Void> resetPassword(@RequestBody ResetKeycloakPasswordCmd resetKeycloakPasswordCmd) {
        authServiceCommand.resetPassword(resetKeycloakPasswordCmd);
        return ApiResponses.<Void>builder()
                .success(true)
                .code(200)
